...
class ModerationServiceTest {
    ...
    ContentReportRepository reports; ListingRepository listings; ModerationService service;
    Listing listing; CurrentUser reporterUser, moderatorUser;

    @BeforeEach void setup() {
        reports = mock(ContentReportRepository.class); listings = mock(ListingRepository.class);
        service = new ModerationService(reports, listings);
        when(reports.saveAndFlush(any())).thenAnswer(inv -> inv.getArgument(0));

        listing = new Listing(); listing.id = UUID.randomUUID(); listing.tenantId = tenant; listing.status = "ACTIVE";
